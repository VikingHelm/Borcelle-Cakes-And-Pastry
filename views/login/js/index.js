...
formulario.addEventListener('submit', async e => {
    ...
    const response = await fetch('http://localhost:3000/api/users/users', {method: 'GET'});
    ...
    displayAlertRed('El usuario o la contraseña son incorrectos.');

    setTimeout(()=>{
    const alertGlobe = document.getElementById('alertGlobeRed');
        
        alertGlobe.classList.add('hidden');
    }, 3000);
...
const displayAlertRed = (message) => {
  ...
  const displayAlertGreen = (message) => {
    const alertGlobe = document.getElementById('alertGlobeGreen');
    alertGlobe.textContent = message;
    alertGlobe.style.display = 'block';
